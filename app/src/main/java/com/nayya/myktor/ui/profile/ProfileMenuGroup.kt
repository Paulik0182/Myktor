...
sealed class ProfileMenuType(
    ...
    val onlyForAdmin: Boolean = false
) {
    object AdminSystem : ProfileMenuType(
        "Администратор системы", R.drawable.ic_admin_system, ProfileMenuGroup.SYSTEM, onlyForAdmin = true
    )

    object AdminStore : ProfileMenuType(
        "Администратор магазина", R.drawable.ic_admin_store, ProfileMenuGroup.SYSTEM, onlyForAdmin = true
    )

    object AdminWarehouse : ProfileMenuType(
        "Администратор склада", R.drawable.ic_admin_warehouse, ProfileMenuGroup.SYSTEM, onlyForAdmin = true
    )

    object Orders : ProfileMenuType("Заказы", R.drawable.ic_orders, ProfileMenuGroup.PURCHASES)
    object Deliveries : ProfileMenuType("Доставки", R.drawable.ic_deliveries, ProfileMenuGroup.PURCHASES)
    object Favorites : ProfileMenuType("Избранное", R.drawable.ic_favorites_border_menu, ProfileMenuGroup.PURCHASES)
    object PromoCodes : ProfileMenuType("Промокоды", R.drawable.ic_promo, ProfileMenuGroup.PURCHASES)
    object DeliveryConditions : ProfileMenuType("Условия доставки", R.drawable.ic_delivery_conditions, ProfileMenuGroup.INFO)
    object Feedback : ProfileMenuType("Отзывы и вопросы", R.drawable.ic_feedback, ProfileMenuGroup.INFO)
    object InfoForUser : ProfileMenuType("Информация для пользователя", R.drawable.ic_info_user, ProfileMenuGroup.INFO)
    object AboutApp : ProfileMenuType("О приложении", R.drawable.ic_about_app, ProfileMenuGroup.SETTINGS)
    object PaymentMethods : ProfileMenuType("Способы оплаты", R.drawable.ic_payment, ProfileMenuGroup.SETTINGS)
    object Settings : ProfileMenuType("Настройки", R.drawable.ic_settings, ProfileMenuGroup.SETTINGS)
    object PrivacyPolicy : ProfileMenuType("Политика конфиденциальности", R.drawable.ic_privacy, ProfileMenuGroup.SETTINGS)

    companion object {
        fun all(): List<ProfileMenuType> = listOf(
            AdminSystem,
            AdminStore,
            AdminWarehouse,
            Orders,
        ...
        fun getVisibleItems(counterparty: CounterpartyEntity, role: UserRole): List<ProfileMenuType> {
            val isAdmin = when (role) {
                UserRole.SYSTEM_ADMIN,
                UserRole.WAREHOUSE_ADMIN,
                UserRole.SHOP_ADMIN -> true
            ...
            return all().filter {
                !it.onlyForAdmin || isAdmin
            }
        }

        fun getVisibleItemsForGuest(): List<ProfileMenuType> {
            return all().filter { !it.onlyForAdmin }
        }
    }
}
